   ...
   for j in range(n_contour):
       ...
       if (min(energia) < levels_n[j] and max(energia) > levels_n[j]):
          ...
          for k in range(passo):
              #----------------------------------------
              if (i == 1):
                 v_spin = [new_Sx[k], 0.0, 0.0]
              if (i == 2):
                 v_spin = [0.0, new_Sy[k], 0.0]
              if (i == 3):
                 v_spin = [0.0, 0.0, new_Sz[k]]
              if (i == 4):
                 v_spin = [Spin_S1[k], Spin_S2[k], 0.0]
              #----------------------------------------   
              v_spin_norm = v_spin / np.linalg.norm(v_spin)
              angle[k] = np.arccos(np.clip(np.dot(v_spin_norm, [1.0, 0.0, 0.0]), -1.0, 1.0))  # Ângulo em radianos
              angle[k] = angle[k]*57.295779513082320876798154814105  # Conversão dos ângulos de radianos para graus.

          #---------------------------------------------------------------------------------------------------------------------------------------
          # map_red_blue = colors.ListedColormap(['red', 'blue','red', 'blue'])
          # boundaries = [0, 90, 180, 270, 360] 
          #---------------------------------------------------------------------------------------------------------------------------------------
          map_red_blue = colors.LinearSegmentedColormap.from_list("", ["red","white","blue","white","red","white","blue","white","red"])
