...
for i in range (1,(NEDOS+1)):
      VTemp = doscar.readline().split()
      energia[i] = float(VTemp[0]); dos[i] = float(VTemp[1]); dos_int[i] = float(VTemp[2])
      energia[i] = (energia[i] - Efermi)
...
# Escrita do arquivo ".agr" do GRACE ===================================
    ...
    for j in range (s,t):
        ...
        if (j == (s+0)): grac='s1'; color = cor_orb[j]        # Cor dos Orbitais (S,Px,Dxy)
        if (j == (s+1)): grac='s2'; color = cor_orb[j]        # Cor dos Orbitais (P,Py,Dyz)
        if (j == (s+2)): grac='s3'; color = cor_orb[j]        # Cor dos Orbitais (D,Pz,Dz2)
        if (j == (s+3)): grac='s4'; color = cor_orb[j]        # Cor do Orbital Dxz
        if (j == (s+4)): grac='s5'; color = cor_orb[j]        # Cor do Orbital Dx2
           
        dos_pdos.write(f'@    {grac} type xy \n')
        dos_pdos.write(f'@    {grac} line type 1 \n')
        dos_pdos.write(f'@    {grac} line color {color} \n')
        dos_pdos.write(f'@    {grac} line linewidth 1.5 \n')
        dos_pdos.write(f'@    {grac} fill type 1 \n')
        dos_pdos.write(f'@    {grac} fill color {color} \n')
        dos_pdos.write(f'@    {grac} fill pattern 4 \n')      
        dos_pdos.write(f'@    {grac} legend  "{r_pdos[j]}" \n')  
    ...
    if (i == 1 and esc == 1):
       dos_pdos.write(f'@    s4 type xy \n')
       dos_pdos.write(f'@    s4 line type 1 \n')
       dos_pdos.write(f'@    s4 line color 10 \n')
       dos_pdos.write(f'@    s4 line linewidth 1.5 \n')
       dos_pdos.write(f'@    s4 fill type 1 \n')
       dos_pdos.write(f'@    s4 fill color 10 \n')
       dos_pdos.write(f'@    s4 fill pattern 4 \n')      
       dos_pdos.write(f'@    s4 legend  "l-DOS" \n')  
...
# Plot da DOS e pDOS ======================================================
    ...
    for l in range(1,(6+1)):
        for k in range (1,(NEDOS+1)):
            ...
            if (i == 1 and l == 2): dos_pdos.write(f'{pdos_tot[1][k]} {energia[k]} \n')
            if (i == 1 and l == 3): dos_pdos.write(f'{pdos_P_tot[k]} {energia[k]} \n')
            if (i == 1 and l == 4): dos_pdos.write(f'{pdos_D_tot[k]} {energia[k]} \n')
            if (esc == 1 and (i == 1 and l == 5)): dos_pdos.write(f'{l_dos[k]} {energia[k]} \n')
            #---------------------------------------------------------------------------
...
for l in range (1,(loop+1)):     # Loop para a analise das Projecoes
    ...
    if (l == 1): 
       plt.savefig('saida/DOS/DOS_PDOS.png', dpi = 300)
       plt.savefig('saida/DOS/DOS_PDOS.pdf', dpi = 300)
       # plt.savefig('saida/DOS/DOS_PDOS.eps', dpi = 300)

    if (l == 2): 
       plt.savefig('saida/DOS/PDOS_P.png', dpi = 300)
       plt.savefig('saida/DOS/PDOS_P.pdf', dpi = 300)
       # plt.savefig('saida/DOS/PDOS_P.eps', dpi = 300)

    if (l == 3): 
       plt.savefig('saida/DOS/PDOS_D.png', dpi = 300)
       plt.savefig('saida/DOS/PDOS_D.pdf', dpi = 300)
       # plt.savefig('saida/DOS/PDOS_D.eps', dpi = 300)
