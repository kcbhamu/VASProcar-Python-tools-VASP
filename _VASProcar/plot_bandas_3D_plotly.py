...
xi = np.linspace(min(eixo1), max(eixo1), num = 101)
yi = np.linspace(min(eixo2), max(eixo2), num = 101)
...
for i in range (1,(Band_f - Band_i +2)):
    ...
    if (tipo_plot == 1):
       fig.add_trace(go.Surface(x = x_grid, y = y_grid, z = z_grid, name = label, opacity = 0.8)) 
    if (tipo_plot == 2):
       fig.add_trace(go.Scatter3d(x = eixo1, y = eixo2, z = energ, name = label, mode = 'markers', marker = dict(size = 2, color = i, opacity = 0.5)))
       fig.add_trace(go.Surface(x = x_grid, y = y_grid, z = z_grid, name = label, opacity = 0.8))                 
