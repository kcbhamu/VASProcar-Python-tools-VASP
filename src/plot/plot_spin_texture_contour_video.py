...
#-----------------------------------------------------------------------
    ...
    if (tipo_spin < 4):   
       cbar = fig.colorbar(cp, location = 'right', orientation = 'vertical', shrink = 1.0, boundaries = [0, 180, 360], ticks = [90, 270])
       cbar.ax.set_yticklabels([L1, L2])

    if (tipo_spin > 3):   
       cbar = fig.colorbar(cp, location = 'right', orientation = 'vertical', shrink = 1.0, values = [45, 90, 135, 180, 225], ticks = [45, 90, 135, 180, 225])
    ...
    plt.title('Band ' + str(Band) + ' (' + c_energ + ' eV)')
    ...
    if (save_png == 1): plt.savefig(dir_output + 'Spin_Texture_video_' + number + '_[' + c_energ + '].png', dpi = 600, pad_inches = 0)
