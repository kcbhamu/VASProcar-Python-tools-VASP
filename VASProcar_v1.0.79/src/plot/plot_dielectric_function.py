...
for i in range(2):
    ...
    if (esc_ef == 1): plt.plot([0.0, 0.0], [y_inicial, y_final], color = 'gray', linestyle = '-', linewidth = 0.1, alpha = 1.0)        
    ...
    if (esc_ef == 0): plt.xlabel('Energia (eV)')
    if (esc_ef == 1): plt.xlabel('$E-E_{f}$ (eV)')
