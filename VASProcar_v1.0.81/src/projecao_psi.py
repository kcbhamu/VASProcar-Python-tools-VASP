...
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# Verificar se um mesmo par (ion,orbital) não foi atribuido a estados Psi distintos
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
ion_select = [[0]*(ni+1) for k in range(5+1)]  #  ion_select[n_psi][ni]
orb_select = [[0]*(n_orb+1) for k in range(5+1)]  #  orb_select[n_psi][n_orb]
psi = [[[[0]*(nb+1) for i in range(nk+1)] for j in range(n_procar+1)] for k in range(5+1)]  #  psi[n_psi][n_procar][nk][nb]          
est_Psi = [[[[0]*(nb+1) for i in range(nk+1)] for j in range(n_procar+1)] for k in range(5+1)]  #  est_Psi[n_psi][n_procar][nk][nb]
estado_Psi = [[[[0]*(nb+1) for i in range(nk+1)] for j in range(n_procar+1)] for k in range(5+1)]  #  estado_Psi[n_psi][n_procar][nk][nb]
total = [[[0]*(nb+1) for j in range(nk+1)] for k in range(n_procar+1)]  #  total[n_procar][nk][nb] 
...
for i in range(1,(n_psi+1)):
    ...
    print ("##############################################################")
    print (f'Informe abaixo quais orbitais compoem o estado Psi {i} ======== ')
    print ("Digite como no exemplo abaixo, onde [0] = NAO e [1] = SIM === ")
    print ("--------------------------------------------------------------")
    if (lorbit == 10):
       print ("[S P D]: 1 1 0                                                ")
    if (lorbit >= 11):
       print ("[S Px Py Pz Dxy Dyz Dz2 Dxz Dx2]: 1 1 1 1 0 0 0 0 0           ")             
    print ("##############################################################")
    print (" ")

    if (lorbit == 10):           
       t_resp = input ("[S P D]: ").split()

    if (lorbit > 10):
       t_resp = input ("[S Px Py Pz Dxy Dyz Dz2 Dxz Dx2]: ").split()
           
    for j in range(1,(n_orb+1)):                  
        orb_select[i][j] = int(t_resp[j-1])             
          
        print (" ")
        print ("##############################################################")
        print (f'Agora voce deve escolher quais ions compoem o estado Psi {i} == ')         
        print ("Selecione os ions por meio de intervalos: =================== ")
        print ("--------------------------------------------------------------")
        print (f'Quantos intervalos deseja fornecer para o estado Psi {i}? ===== ')
        print ("##############################################################") 
        loop = input (" "); loop = int(loop)

        print (" ")
        print ("##############################################################")          
        print ("Informe os intervalos de ions como nos exemplos abaixo ====== ")
        print ("--------------------------------------------------------------")
        ...
        for k in range (1,(loop+1)):
            ...
            for m in range(loop_i, (loop_f+1)):
                ion_select[i][m] = 1            
        ...
        for Band_n in range (1, (nb+1)):
            for ion_n in range (1, (ni+1)):                                       
                for orb_n in range(1,(n_orb+1)):
                    ...
                    #-----------------------------------------------------------------------------------------------
                    for psi in range(1,(n_psi+1)):
                        if (ion_select[psi][ion_n] == 1 and orb_select[psi][orb_n] == 1):
                           psi[psi][wp][point_k][Band_n] = psi[psi][wp][point_k][Band_n] + orb[wp][orb_n][point_k][Band_n][ion_n]                
            #----------------------------------------------------------           
            # Fim do Loop dos ions ------------------------------------
            #----------------------------------------------------------  
            for psi in range (1,(n_psi+1)):
                if (total[wp][point_k][Band_n] != 0.0):
                   psi[psi][wp][point_k][Band_n] = ( psi[psi][wp][point_k][Band_n]/total[wp][point_k][Band_n] )*peso_total                 
        #----------------------------------------------------------
...
for j in range (1,(n_psi+1)):
    ...
    if (j == 1): grac='s0'; legend = '\\f{Symbol}y 1'; color = cor_orb[j+5]  #  Cor do estado Psi 1
    if (j == 2): grac='s1'; legend = '\\f{Symbol}y 2'; color = cor_orb[j+5]  #  Cor do estado Psi 2
    if (j == 3): grac='s2'; legend = '\\f{Symbol}y 3'; color = cor_orb[j+5]  #  Cor do estado Psi 3
    if (j == 4): grac='s3'; legend = '\\f{Symbol}y 4'; color = cor_orb[j+5]  #  Cor do estado Psi 4
    if (j == 5): grac='s4'; legend = '\\f{Symbol}y 5'; color = cor_orb[j+5]  #  Cor do estado Psi 5
...
for k in range (1,(nb+1)):
    for i in range (1,(n_procar+1)):
        for j in range (1,(nk+1)):
            ...
            orbital.write(f' {estado_Psi[1][i][j][k]} {estado_Psi[2][i][j][k]} {estado_Psi[3][i][j][k]} {estado_Psi[4][i][j][k]} {estado_Psi[5][i][j][k]}')         
            orbital.write(f' \n')
...
for Band_n in range (1, (nb+1)):
    for wp in range (1, (n_procar+1)):
        for point_k in range (1, (nk+1)):       
           ...
           #-----------------------------------------------------------------------------               
           c_red    =  estado_Psi[2][wp][point_k][Band_n] + 0.737254902*(estado_Psi[5][wp][point_k][Band_n]) + estado_Psi[4][wp][point_k][Band_n]
           c_green  =  estado_Psi[3][wp][point_k][Band_n] + 0.560784313*(estado_Psi[5][wp][point_k][Band_n])
           c_blue   =  estado_Psi[1][wp][point_k][Band_n] + 0.560784313*(estado_Psi[5][wp][point_k][Band_n]) + estado_Psi[4][wp][point_k][Band_n]
           #--------------------------------
