...
if (escolha == -2):
   ...
   print ("Utilize 1 para k em unidades de dpi/Param com Param em Angs. =")
...
# Escrita do arquivo ".agr" do GRACE ===================================
    ...
    if (Dimensao == 1 and dest_k != 2): projection.write(f'@    xaxis  label "(2pi/Param.)" \n') 
    ...
    if (dest_k == 2):
       ...
       for i in range (contador2):   
           ...
           projection.write(f'@    xaxis  ticklabel {i}, "{label_pk[i]}" \n')
...
for l in range (1,(loop+1)):     # Loop para a analise das Projecoes
    ...
    for k in range (1,(nb+1)):
        for i in range (1,(n_procar+1)):
            for j in range (1,(nk+1)):
                ...
                if (l == 1):
                   S[number] = ((dpi*soma_orb[i][1][j][k])**2)*peso_total

                if (l == 1 and lorbit == 10):
                   P[number] = ((dpi*soma_orb[i][2][j][k])**2)*peso_total
                   D[number] = ((dpi*soma_orb[i][3][j][k])**2)*peso_total

                if (l == 1 and lorbit >= 11):
                   P[number] = ((dpi*(soma_orb[i][2][j][k] + soma_orb[i][3][j][k] + soma_orb[i][4][j][k]))**2)*peso_total
                   D[number] = ((dpi*(soma_orb[i][5][j][k] + soma_orb[i][6][j][k] + soma_orb[i][7][j][k] + soma_orb[i][8][j][k] + soma_orb[i][9][j][k]))**2)*peso_total            
                      
                if (l == 2):
                   Px[number] = ((dpi*soma_orb[i][4][j][k])**2)*peso_total
                   Py[number] = ((dpi*soma_orb[i][2][j][k])**2)*peso_total
                   Pz[number] = ((dpi*soma_orb[i][3][j][k])**2)*peso_total
                                         
                if (l == 3):
                   Dxy[number] = ((dpi*soma_orb[i][5][j][k])**2)*peso_total
                   Dyz[number] = ((dpi*soma_orb[i][6][j][k])**2)*peso_total
                   Dz2[number] = ((dpi*soma_orb[i][7][j][k])**2)*peso_total
                   Dxz[number] = ((dpi*soma_orb[i][8][j][k])**2)*peso_total
                   Dx2[number] = ((dpi*soma_orb[i][9][j][k])**2)*peso_total
    ...
    if (l == 1): 
       if (save_png == 1): plt.savefig('saida/Orbitais/Orbitais_S_P_D.png', dpi = 300)
       if (save_pdf == 1): plt.savefig('saida/Orbitais/Orbitais_S_P_D.pdf', dpi = 300)
       if (save_eps == 1): plt.savefig('saida/Orbitais/Orbitais_S_P_D.eps', dpi = 300)

    if (l == 2): 
       if (save_png == 1): plt.savefig('saida/Orbitais/Orbitais_P.png', dpi = 300)
       if (save_pdf == 1): plt.savefig('saida/Orbitais/Orbitais_P.pdf', dpi = 300)
       if (save_eps == 1): plt.savefig('saida/Orbitais/Orbitais_P.eps', dpi = 300)

    if (l == 3): 
       if (save_png == 1): plt.savefig('saida/Orbitais/Orbitais_D.png', dpi = 300)
       if (save_pdf == 1): plt.savefig('saida/Orbitais/Orbitais_D.pdf', dpi = 300)
       if (save_eps == 1): plt.savefig('saida/Orbitais/Orbitais_D.eps', dpi = 300)
