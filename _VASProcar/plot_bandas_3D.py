...
####################################################################################################################################
# escolha   = 6    # [6] Plot Banda 3D  ou  Plot Banda 2D em 3D                                                                   ##
...
# tipo_plot = ???  # [0] Plotado em pontos // [1] Plotado superficies // [2] pontos + superficies                                 ##
####################################################################################################################################
...
banda = np.loadtxt("saida/Bandas_3D/Bandas_3D.dat")
banda.shape
...
for i in range (Band_i,(Band_f+1)):
    ...
    if ((escolha == 6 or escolha == -6) and tipo_plot == 2):
       ax.plot_trisurf(eixo1, eixo2, E[number], alpha = 0.9, cmap = 'coolwarm', edgecolor='black', linewidth = 0.0, antialiased = False)
       ax.scatter(eixo1, eixo2, E[number], s = 0.1, alpha = 0.25, color = 'gray', antialiased = False)
...
plt.savefig('saida/Bandas_3D/Bandas_3D.png', dpi = 300, pad_inches = 0)
plt.savefig('saida/Bandas_3D/Bandas_3D.pdf', dpi = 300, pad_inches = 0)
# plt.savefig('saida/Bandas_3D/Bandas_3D.eps', dpi = 300, pad_inches = 0)

plt.show()

#----------------------------------------------------------------------------------------
# Verificando se o arquivo Plot_Bandas_3D.py já foi copiado para o diretório principal --
#----------------------------------------------------------------------------------------

try: f = open('saida/Bandas_3D/Plot_Bandas_3D.py'); f.close(); Plot_3D = 1
except: Plot_3D = 0   

if (Plot_3D == 0 ):
   source = Diretorio + '\saida_plot_bandas_3D.py'
   destination = r'saida/Bandas_3D\Plot_Bandas_3D.py'
