    ...
    for k in range (1,(nb+1)):
        for i in range (1,(n_procar+1)):
            for j in range (1,(nk+1)):
                ...
                if (l == 1):
                   if (tot_sx[i][j][k] > 0): Si_u[number] = ((tot_sx[i][j][k])*(peso_total**0.5)*6)**2
                   if (tot_sx[i][j][k] < 0): Si_d[number] = ((tot_sx[i][j][k])*(peso_total**0.5)*6)**2
                      
                if (l == 2):
                   if (tot_sy[i][j][k] > 0): Si_u[number] = ((tot_sy[i][j][k])*(peso_total**0.5)*6)**2
                   if (tot_sy[i][j][k] < 0): Si_d[number] = ((tot_sy[i][j][k])*(peso_total**0.5)*6)**2
                      
                if (l == 3):
                   if (tot_sz[i][j][k] > 0): Si_u[number] = ((tot_sz[i][j][k])*(peso_total**0.5)*6)**2
                   if (tot_sz[i][j][k] < 0): Si_d[number] = ((tot_sz[i][j][k])*(peso_total**0.5)*6)**2      

    ax.scatter(rx, ry, s = Si_u, c = 'red',  alpha = 1.0, edgecolors = 'none', label = r'$\uparrow$')
    ax.scatter(rx, ry, s = Si_d, c = 'blue', alpha = 1.0, edgecolors = 'none', label = r'$\downarrow$')
