...
for k in range (1,(nb+1)):
    for i in range (1,(n_procar+1)):
        for j in range (1,(nk+1)):
            ...
            ra[number] = ((Prop[1][i][j][k])*(peso_total**0.5)*6)**2
            rb[number] = ((Prop[2][i][j][k])*(peso_total**0.5)*6)**2
            rc[number] = ((Prop[3][i][j][k])*(peso_total**0.5)*6)**2
            rd[number] = ((Prop[4][i][j][k])*(peso_total**0.5)*6)**2
            re[number] = ((Prop[5][i][j][k])*(peso_total**0.5)*6)**2

if (num_A == 1): ax.scatter(rx, ry, s = ra, c = 'blue', alpha = 1.0, edgecolors = 'none', label = 'A')
if (num_B == 1): ax.scatter(rx, ry, s = rb, c = 'red', alpha = 1.0, edgecolors = 'none', label = 'B')
if (num_C == 1): ax.scatter(rx, ry, s = rc, c = 'limegreen', alpha = 1.0, edgecolors = 'none', label = 'C')
if (num_D == 1): ax.scatter(rx, ry, s = rd, c = 'rosybrown', alpha = 1.0, edgecolors = 'none', label = 'D')
if (num_E == 1): ax.scatter(rx, ry, s = re, c = 'magenta', alpha = 1.0, edgecolors = 'none', label = 'E')
