...
# Escrita do arquivo ".agr" do GRACE ===================================
    ...
    for i in range (1,(3+1)):
        ...
        if (i == 1):
           grac='s0'; color = cor_spin[2]; legenda = '\\f{Symbol}\\c-\\C'   # Cor (Vermelho) da componente Up dos Spins Sx, Sy e Sz.
        if (i == 2):
           grac='s1'; color = cor_spin[3]; legenda = '\\f{Symbol}\\c/\\C'   # Cor (Azul) da componente Down dos Spins Sx, Sy e Sz.
        if (i == 3):
...
for l in range (1,(3+1)):
    ...
    if (l == 1): 
       if (save_png == 1): plt.savefig('saida/Spin/Spin_Sx.png', dpi = 300)
       if (save_pdf == 1): plt.savefig('saida/Spin/Spin_Sx.pdf', dpi = 300)
       if (save_eps == 1): plt.savefig('saida/Spin/Spin_Sx.eps', dpi = 300)

    if (l == 2): 
       if (save_png == 1): plt.savefig('saida/Spin/Spin_Sy.png', dpi = 300)
       if (save_pdf == 1): plt.savefig('saida/Spin/Spin_Sy.pdf', dpi = 300)
       if (save_eps == 1): plt.savefig('saida/Spin/Spin_Sy.eps', dpi = 300)

    if (l == 3): 
       if (save_png == 1): plt.savefig('saida/Spin/Spin_Sz.png', dpi = 300)
       if (save_pdf == 1): plt.savefig('saida/Spin/Spin_Sz.pdf', dpi = 300)
       if (save_eps == 1): plt.savefig('saida/Spin/Spin_Sz.eps', dpi = 300)
